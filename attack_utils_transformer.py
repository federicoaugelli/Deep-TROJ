...
def flatten_weight_f_extractor(model, unit_size=128):
...
def replace_weight_blocks(model, original_model):
    for m, o_m in zip(model.modules(), original_model.modules()):
        if (isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear)) and (m.mask.sum() > 0):
            mask = m.mask
...
def freeze_classifier(model):
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Linear) and name == last_layer_name:
...
def see_layer_info(model):
    ...
    for name, m in model.named_modules():
        if isinstance(m, nn.Conv2d) or isinstance(m, nn.Linear):
            ...
            if isinstance(m, nn.Linear) and m.weight.requires_grad and name == last_layer_name:
